...
const DoctorApplications = () => {
  ...
  const getAllApp = async (e) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/admin/getnotdoctors`, {
        method: "get",
        headers: {
  ...
  const rejectDoctor = async (userId) => {
    try {
      ...
      if (confirm) {
        const response = await fetch(`${BASE_URL}/admin/rejectdoctor`, {
          method: "put",
          headers: {
            "Content-Type": "application/json",
  ...
  const acceptDoctor = async (doctorId) => {
    try {
      ...
      if (confirm) {
        const response = await fetch(`${BASE_URL}/admin/acceptdoctor`, {
          method: "put",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        if (!response.ok) {
          throw new Error("Unable to accept application");
        }

        // Assuming you have logic to handle successful acceptance
        getAllApp();
      }
    } catch (error) {
      ...
      {applications.length > 0 ? (
        <div className="user-container ">
          ...
          <p className="text-center">
            <Link className="text-primaryColor m-8 text-[20px] font-bold uppercase" to={"https://www.nmc.org.in/information-desk/indian-medical-register/"} target="__blank">Indian Medical Registry Search</Link>
          </p>
        </div>
      ) : (
        <div className="text-center text-primaryColor">No Pending Doctors</div>
      )}
    </section>
  );
};
